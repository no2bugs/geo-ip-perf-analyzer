document.addEventListener('DOMContentLoaded', () => {
    ...
    const configScheduleInfo = document.getElementById('configScheduleInfo');
    ...
    function populateConfigForm(config) {
        const scan = config.schedule?.scan || {};
        document.getElementById('cfgScanEnabled').checked = scan.enabled || false;
        document.getElementById('cfgScanInterval').value = scan.interval || 'daily';
        document.getElementById('cfgScanDay').value = scan.day || 'monday';
        document.getElementById('cfgScanTime').value = scan.time || '03:00';
        document.getElementById('cfgScanPings').value = scan.pings || 1;
        document.getElementById('cfgScanTimeout').value = scan.timeout || 1000;
        document.getElementById('cfgScanWorkers').value = scan.workers || 20;
        ...
        document.getElementById('cfgOvpnEnabled').checked = ovpn.enabled || false;
        ...
        document.getElementById('cfgEvtScanComplete').checked = ntfy.events?.scan_complete !== false;
        document.getElementById('cfgEvtScanError').checked = ntfy.events?.scan_error !== false;
    ...
    function updateConfigVisibility() {
        const scanEnabled = document.getElementById('cfgScanEnabled').checked;
        const geoEnabled = document.getElementById('cfgGeoEnabled').checked;
        const ovpnEnabled = document.getElementById('cfgOvpnEnabled').checked;
        const ntfyEnabled = document.getElementById('cfgNtfyEnabled').checked;

        document.getElementById('cfgScanOptions').style.display = scanEnabled ? '' : 'none';
        document.getElementById('cfgGeoOptions').style.display = geoEnabled ? '' : 'none';
        document.getElementById('cfgOvpnOptions').style.display = ovpnEnabled ? '' : 'none';
        document.getElementById('cfgNtfyOptions').style.display = ntfyEnabled ? '' : 'none';

        document.getElementById('cfgScanDayWrap').style.display =
            document.getElementById('cfgScanInterval').value === 'weekly' ? '' : 'none';
        document.getElementById('cfgGeoDayWrap').style.display =
            document.getElementById('cfgGeoInterval').value === 'weekly' ? '' : 'none';
        document.getElementById('cfgOvpnDayWrap').style.display =
            document.getElementById('cfgOvpnInterval').value === 'weekly' ? '' : 'none';

        // Update schedule summary in footer
        const parts = [];
        if (scanEnabled) parts.push('Scan');
        if (geoEnabled) parts.push('GeoLite2');
        if (ovpnEnabled) parts.push('OVPN');
        configScheduleInfo.textContent = parts.length ? `Active: ${parts.join(', ')}` : 'No schedules active';
    }

    // Attach visibility toggle listeners
    ['cfgScanEnabled', 'cfgGeoEnabled', 'cfgOvpnEnabled', 'cfgNtfyEnabled'].forEach(id => {
        document.getElementById(id).addEventListener('change', updateConfigVisibility);
    });
    ['cfgScanInterval', 'cfgGeoInterval', 'cfgOvpnInterval'].forEach(id => {
        document.getElementById(id).addEventListener('change', updateConfigVisibility);
    });
    ...
    function collectConfig() {
        return {
            schedule: {
                scan: {
                    enabled: document.getElementById('cfgScanEnabled').checked,
                    interval: document.getElementById('cfgScanInterval').value,
                    day: document.getElementById('cfgScanDay').value,
                    time: document.getElementById('cfgScanTime').value,
                    pings: parseInt(document.getElementById('cfgScanPings').value) || 1,
                    timeout: parseInt(document.getElementById('cfgScanTimeout').value) || 1000,
                    workers: parseInt(document.getElementById('cfgScanWorkers').value) || 20
                },
                geolite_update: {
                ...
                ovpn_update: {
                    enabled: document.getElementById('cfgOvpnEnabled').checked,
            ...
            notifications: {
                ntfy: {
                    ...
                    events: {
                        scan_complete: document.getElementById('cfgEvtScanComplete').checked,
                        scan_error: document.getElementById('cfgEvtScanError').checked,
